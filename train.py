...
def train_model(
    ...
    weights = get_balanced_class_weights(use_balanced_class_weights, trainsets).to(device)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for state, loaders in zip(["train", "test"], [train_loaders, test_loaders]):
                ...
                for id, batch in enumerate(loader, 0):
                    ...
                    with torch.set_grad_enabled(state == 'train'):
                        ...
                        for n_class in range(outputs.shape[1]):
                            ...
                            all_predictions = torch.where(argmaxed_proba == n_class, 1, 0).sum().item()
                            ...
                            all_observations_to_be_guessed = torch.where(argmaxed_split_mask == n_class, 1, 0).sum().item()
                            
                            stats[n_class] = [
                                stats[n_class][0], 
                                stats[n_class][1] + correct_predictions, 
                                stats[n_class][2] + all_predictions, 
                                stats[n_class][3] + all_observations_to_be_guessed]
            ...
            for key, value in stats.items():
                ...
                logging.info(f"Class name: {class_name}, precission: {class_precission}, recall: {class_recall}, All observations: {class_observations}")
